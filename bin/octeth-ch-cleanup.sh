#!/bin/bash
#
# Octeth ClickHouse Backup Cleanup Script
...
cleanup_s3_backups() {
    log_info "Cleaning up S3 ClickHouse backups"
...
cleanup_s3_with_aws_cli() {
    ...
    for backup_type in daily weekly monthly; do
        ...
        case "$backup_type" in
            daily)   retention_count=${RETENTION_DAILY} ;;
            weekly)  retention_count=${RETENTION_WEEKLY} ;;
            monthly) retention_count=${RETENTION_MONTHLY} ;;
        esac

        log_info "Cleaning up S3 ${backup_type} ClickHouse backups (keep last ${retention_count})"
...
cleanup_gcs_backups() {
    log_info "Cleaning up GCS ClickHouse backups"
...
cleanup_gcs_with_gsutil() {
    ...
    for backup_type in daily weekly monthly; do
        ...
        log_info "Cleaning up GCS ${backup_type} ClickHouse backups (keep last ${retention_count})"
...
cleanup_r2_backups() {
    log_info "Cleaning up R2 ClickHouse backups"
...
cleanup_r2_with_aws_cli() {
    ...
    for backup_type in daily weekly monthly; do
        ...
        log_info "Cleaning up R2 ${backup_type} ClickHouse backups (keep last ${retention_count})"
...
cleanup_old_logs() {
    local log_dir=$(dirname "${CH_LOG_FILE}")

    if [ ! -d "$log_dir" ]; then
        return 0
    fi

    log_info "Cleaning up old ClickHouse backup log files (older than ${LOG_RETENTION_DAYS} days)"
    ...
    done < <(find "$log_dir" -name "octeth-ch-*.log" -type f -mtime +${LOG_RETENTION_DAYS} -print0)
...
show_statistics() {
    log_info "=========================================="
    log_info "ClickHouse Backup Statistics"
    log_info "=========================================="
    ...
    if [ -d "${CH_BACKUP_DIR}" ]; then
        local total_size=$(du -sh "${CH_BACKUP_DIR}" 2>/dev/null | cut -f1 || echo "0")
        log_info "Total ClickHouse backup size: ${total_size}"
    fi
...
main() {
    ...
    log_info "=========================================="
    log_info "Octeth ClickHouse Backup Cleanup"
    log_info "=========================================="
    ...
    cleanup_directory "${CH_DAILY_DIR}" "${RETENTION_DAILY}" "daily"
    cleanup_directory "${CH_WEEKLY_DIR}" "${RETENTION_WEEKLY}" "weekly"
    cleanup_directory "${CH_MONTHLY_DIR}" "${RETENTION_MONTHLY}" "monthly"
    ...
    log_success "ClickHouse backup cleanup completed"
}
