#!/bin/bash
#
# Octeth ClickHouse Restore Script
# Restores ClickHouse database from clickhouse-backup backups
#
# Author: Octeth Team
# License: MIT
#
...
list_local_backups() {
    log_info "=========================================="
    log_info "Available Local ClickHouse Backups"
    log_info "=========================================="

    local found=0

    for backup_type in daily weekly monthly; do
        local dir_var="CH_$(to_upper "$backup_type")_DIR"
        local dir="${!dir_var}"
    ...
    if [ $found -eq 0 ]; then
        echo "No local ClickHouse backups found"
    else
        echo ""
...
list_s3_backups() {
    log_info "=========================================="
    log_info "Available S3 ClickHouse Backups"
    log_info "=========================================="
...
list_s3_with_aws_cli() {
    ...
    for backup_type in daily weekly monthly; do
        local s3_prefix="${S3_PREFIX}/${CH_CLOUD_SUBDIR}/${backup_type}/"
...
list_s3_with_rclone() {
    ...
    for backup_type in daily weekly monthly; do
        local remote_path="${RCLONE_REMOTE}:${S3_BUCKET}/${S3_PREFIX}/${CH_CLOUD_SUBDIR}/${backup_type}/"
...
list_gcs_backups() {
    log_info "=========================================="
    log_info "Available GCS ClickHouse Backups"
    log_info "=========================================="
...
list_gcs_with_gsutil() {
    ...
    for backup_type in daily weekly monthly; do
        local gcs_prefix="gs://${GCS_BUCKET}/${GCS_PREFIX}/${CH_CLOUD_SUBDIR}/${backup_type}/"
...
list_gcs_with_rclone() {
    ...
    for backup_type in daily weekly monthly; do
        local remote_path="${GCS_RCLONE_REMOTE}:${GCS_BUCKET}/${GCS_PREFIX}/${CH_CLOUD_SUBDIR}/${backup_type}/"
...
# ============================================
# R2 Listing Functions
# ============================================

list_r2_backups() {
    log_info "=========================================="
    log_info "Available R2 ClickHouse Backups"
    log_info "=========================================="
...
list_r2_with_aws_cli() {
    ...
    for backup_type in daily weekly monthly; do
        local r2_prefix="${R2_PREFIX}/${CH_CLOUD_SUBDIR}/${backup_type}/"
...
list_r2_with_rclone() {
    ...
    for backup_type in daily weekly monthly; do
        local remote_path="${R2_RCLONE_REMOTE}:${R2_BUCKET}/${R2_PREFIX}/${CH_CLOUD_SUBDIR}/${backup_type}/"
...
download_from_s3() {
    local backup_name="$1"
    local backup_type="$2"
    local dest_dir="${CH_TEMP_DIR}/restore"
    ...
    if [ "${S3_UPLOAD_TOOL}" = "awscli" ]; then
        download_s3_with_aws_cli "$backup_name" "$backup_type" "$dest_dir"
    elif [ "${S3_UPLOAD_TOOL}" = "rclone" ]; then
        download_s3_with_rclone "$backup_name" "$backup_type" "$dest_dir"
    fi

    echo "${dest_dir}/${backup_name}"
}

download_s3_with_aws_cli() {
    local backup_name="$1"
    local backup_type="$2"
    local dest_dir="$3"
    ...
    local s3_path="s3://${S3_BUCKET}/${S3_PREFIX}/${CH_CLOUD_SUBDIR}/${backup_type}/${backup_name}"
...
download_s3_with_rclone() {
    ...
    local remote_path="${RCLONE_REMOTE}:${S3_BUCKET}/${S3_PREFIX}/${CH_CLOUD_SUBDIR}/${backup_type}/${backup_name}"
...
download_gcs_with_gsutil() {
    ...
    local gcs_path="gs://${GCS_BUCKET}/${GCS_PREFIX}/${CH_CLOUD_SUBDIR}/${backup_type}/${backup_name}"
...
download_gcs_with_rclone() {
    ...
    local remote_path="${GCS_RCLONE_REMOTE}:${GCS_BUCKET}/${GCS_PREFIX}/${CH_CLOUD_SUBDIR}/${backup_type}/${backup_name}"
...
download_r2_with_aws_cli() {
    ...
    local r2_endpoint="https://${R2_ACCOUNT_ID}.r2.cloudflarestorage.com"
    local r2_path="s3://${R2_BUCKET}/${R2_PREFIX}/${CH_CLOUD_SUBDIR}/${backup_type}/${backup_name}"
...
download_r2_with_rclone() {
    ...
    local remote_path="${R2_RCLONE_REMOTE}:${R2_BUCKET}/${R2_PREFIX}/${CH_CLOUD_SUBDIR}/${backup_type}/${backup_name}"
...
perform_restore() {
    ...
    if [ "$SKIP_CONFIRMATION" = false ]; then
        echo ""
        log_warn "=========================================="
        log_warn "WARNING: This will RESTORE data into your ClickHouse database!"
        log_warn "Database: ${CH_DATABASE} on ${CH_HOST}"
        log_warn "Restore from: $(basename $backup_file)"
        if [ "$DROP_BEFORE_RESTORE" = true ]; then
            log_warn "MODE: Tables will be DROPPED before restore"
        else
            log_warn "MODE: Existing tables will be preserved (restore may fail if tables exist)"
        fi
        log_warn "=========================================="
        echo ""
    ...
    # Extract backup
    local extract_dir="${CH_TEMP_DIR}/restore-$(date +%s)"
    mkdir -p "$extract_dir"
    ...
    local backup_dir=$(find "$extract_dir" -maxdepth 1 -type d -name "${CH_BACKUP_PREFIX}-*" | head -n1)
    ...
    if [ "$DROP_BEFORE_RESTORE" = true ]; then
        log_info "Dropping existing tables in database ${CH_DATABASE}..."
        local tables=$(${DOCKER_CMD} exec ${CH_HOST} clickhouse-client \
            --user="${CH_USER}" ${CH_PASSWORD:+--password="${CH_PASSWORD}"} \
            --query="SELECT name FROM system.tables WHERE database='${CH_DATABASE}'" 2>/dev/null || echo "")

        if [ -n "$tables" ]; then
            while IFS= read -r table; do
                ${DOCKER_CMD} exec ${CH_HOST} clickhouse-client \
                    --user="${CH_USER}" ${CH_PASSWORD:+--password="${CH_PASSWORD}"} \
                    --query="DROP TABLE IF EXISTS ${CH_DATABASE}.${table}" 2>/dev/null || \
                    log_warn "Failed to drop table: ${table}"
            done <<< "$tables"
    ...
    log_info "Restoring backup with clickhouse-backup..."

    local restore_opts=""
    if [ "$DROP_BEFORE_RESTORE" = false ]; then
        # Allow restoring into existing tables (schema may already exist)
    ...
    local table_count=$(${DOCKER_CMD} exec ${CH_HOST} clickhouse-client \
        --user="${CH_USER}" ${CH_PASSWORD:+--password="${CH_PASSWORD}"} \
        --query="SELECT count() FROM system.tables WHERE database='${CH_DATABASE}'" 2>/dev/null || echo "0")

    log_success "=========================================="
    log_success "Restore completed successfully!"
    log_success "Database: ${CH_DATABASE}"
...
OPTIONS:
    ...
    -s, --s3 NAME TYPE      Restore from S3 backup
    ...
    -D, --drop              Drop existing tables before restore
...
EXAMPLES:
    ...
    $(basename $0) --file /var/backups/octeth-ch/daily/octeth-ch-backup-2025-01-15_02-00-00.tar.gz

    # Restore from cloud backup (uses CLOUD_STORAGE_PROVIDER from config)
    $(basename $0) --cloud octeth-ch-backup-2025-01-15_02-00-00.tar.gz daily

    # Restore with drop (clean restore)
    $(basename $0) --file backup.tar.gz --drop --yes
...
main() {
    ...
    log_info "=========================================="
    log_info "Octeth ClickHouse Restore"
    log_info "=========================================="
